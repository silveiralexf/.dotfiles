...
return {
  ...
  config = function()
    ...
    if type(neotest) ~= 'table' or not neotest.setup then return end
    local adapters = {}
    ...
    if plenary_ok and plenary then adapters[#adapters + 1] = plenary end
    local vimtest_ok, vimtest = pcall(require, 'neotest-vim-test')
    if vimtest_ok and vimtest then adapters[#adapters + 1] = vimtest end
