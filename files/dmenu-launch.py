...
Requirements
---------------
  - dmenu, gpg, pass, xclip, xdg-open, pkill
Usage
---------------
...
---------------
  ...
  --apps      Quick launches a desktop application with xdg-open.
  --notes     Opens a text/markdown note from a given directory with xdg-open.
  --search    Quick search and launch from a given directory with xdg-open.
Contact / Links
---------------
...
def check_req_utils():
    ...
    utils = (['dmenu', 'gpg', 'pass', 'xclip', 'xdg-open', 'pkill'])
    for util in utils:
...
def get_args():
    ...
    group.add_argument('--apps', action='store_true',
                        help='Quick launches a desktop application with xdg-open.')
    group.add_argument('--notes', action='store_true',
                        help='Opens a text/markdown note from a given directory with xdg-open.')
    group.add_argument('--search', action='store_true',
                        help='Quick search and launch from a given directory with xdg-open.')
...
def take_action(scheme, choice):
    ...
    if (scheme.target == "notes"):
        run_subprocess('code ~/notes/zettelkasten.code-workspace "{}"'.format(choice))

    if (scheme.target == "apps") or (scheme.target == "search"):
        run_subprocess('xdg-open "{}"'.format(choice))
